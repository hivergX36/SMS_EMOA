...
function INDICATOR_is_dominating(sol1::Sol, sol2::Sol; debug = true)
...
function INDICATOR_fast_non_dominated_sort!(population::Vector{Sol}; debug = true)

    debug && DEBUG_feasible_solutions(population)

    @timeit to "Indicator.fast_non_dominated_sort!" begin
    ...
    for idx_p in 1:n
        ...
        for idx_q in idx_p+1:n
            q = population[idx_q]
            if INDICATOR_is_dominating(p, q, debug = debug)
                push!(S[idx_p], idx_q)
                domination_count[idx_q] += 1
            elseif INDICATOR_is_dominating(q, p, debug = debug)
                push!(S[idx_q], idx_p)
...
function INDICATOR_crowding_distance!(front::Vector{Sol}; debug = true)

    debug && DEBUG_feasible_solutions(front)
    debug && DEBUG_is_front(front)

    @timeit to "Indicator.crowding_distance" begin

    n = length(front)

    if n == 1

        front[1].crowding = TOOL_inf()
        ...
        front[perm[1]].crowding = TOOL_inf()
        front[perm[n]].crowding = TOOL_inf()
...
# ASSERTION : front is a front, hence solutions are mutually non dominated
function INDICATOR_hypervolume_contribution!(front::Vector{Sol}; debug = true)
    ...
    front[perm[1]].hv_contrib = TOOL_inf()
    front[perm[N]].hv_contrib = TOOL_inf()
