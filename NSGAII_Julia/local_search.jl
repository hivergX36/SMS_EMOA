...
function local_voisin(pop::Sol,nb_voisin::Int,item)
    p = copy(item)
    voisins = []
    capacite = pop.prob.ω
...
function local_search(momkp::_MOMKP,population::Vector{Sol}, nbvoisin)
    ...
    for i in 1:length(choix) 
        ...
        item = individu.x
        voisin = local_voisin(individu,nbvoisin,item)
        if length(voisin) > 0
