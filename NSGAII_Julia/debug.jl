...
function DEBUG_correct_evaluations(population::Vector{Sol}; crowding = false, hv = false)
    ...
    F = INDICATOR_fast_non_dominated_sort!(pop_copy, debug = false)
    for front in F
        crowding && INDICATOR_crowding_distance!(front, debug = false)
        hv       && INDICATOR_hypervolume_contribution!(front, debug = false)
    end
